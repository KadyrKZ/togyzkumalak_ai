...
class Board:
    ...
    def move(self, action):
        ...
        if opponent_side:
            # взятие чётных (правило взятия)
            if home[otau].kumalaks % 2 == 0:
                self.take(home[otau])

            # --- TUZDYK rules (строго) ---
            can_make_tuzdyk = (
                home[otau].kumalaks == 3          # 2 -> 3 (правило "числа три")
                and otau != 8                      # запрет на 9-ю лунку (индекс 8)
                and not self.run.tuzduk_announced  # однократность
                and not self.run.home[otau].tuzduk # запрет "одинаковых номеров"
                and not home[otau].tuzduk          # на всякий случай: лунка не туздык уже
            )

            if can_make_tuzdyk:
                home[otau].tuzduk = True
                self.run.tuzduk_announced = True
                self.take(home[otau])              # сразу забрать 3 в казан
                pass
            pass
